...
|---|---|---|
| `enum TransitMode { Bus=0, Rail=1 }` + `RouteNearestPointRecord.TransitMode` at `Key(10)` | **Confirmed.** `src/ChefKnifeStudios.MartaJazz.Shared/Events/RouteNearestPointBatchEvent.cs:7,44` — `[property: Key(10)] TransitMode TransitMode = TransitMode.Bus`. | Retype to `[property: Key(10)] string Category = "bus"`; remove the enum. |
...
| Real path prefix | Actual: `src/ChefKnifeStudios.MartaJazz.Shared/`, `src/Server/ChefKnifeStudios.MartaJazz.Server.WebAPI/`, `src/Client/ChefKnifeStudios.MartaJazz.Client.Shared/` — **not** the doc's `src/Shared/...`. | Plan uses real globbed paths; tasks must too. |
