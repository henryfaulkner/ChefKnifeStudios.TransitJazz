...
**Namespace note**: product name is `TransitJazz`; real source root is `ChefKnifeStudios.TransitJazz`
...
Web application (constitution Principle I): `src/ChefKnifeStudios.TransitJazz.Shared/`,
`src/Server/ChefKnifeStudios.TransitJazz.Server.TransitDataWorker/`,
`src/Client/ChefKnifeStudios.TransitJazz.Client.Shared/`,
`src/Client/ChefKnifeStudios.TransitJazz.Client.WebApp/`. Tests in
`src/Server/ChefKnifeStudios.TransitJazz.Server.TransitDataWorker.Tests/` (existing xUnit project).
...
- [X] T002 Move `TriggerPoint` to `src/ChefKnifeStudios.TransitJazz.Shared/Models/TriggerPoint.cs` — copy the record verbatim, change namespace to `ChefKnifeStudios.TransitJazz.Shared.Models`, then delete `src/Client/ChefKnifeStudios.TransitJazz.Client.Shared/Models/TriggerPoint.cs`.
- [X] T003 Move `ITriggerPointGenerator` to `src/ChefKnifeStudios.TransitJazz.Shared/Services/ITriggerPointGenerator.cs` — namespace `ChefKnifeStudios.TransitJazz.Shared.Services`, update its `using` for `TriggerPoint`; delete `src/Client/ChefKnifeStudios.TransitJazz.Client.Shared/Services/ITriggerPointGenerator.cs` (depends on T002).
- [X] T004 Move `TriggerPointGenerator` to `src/ChefKnifeStudios.TransitJazz.Shared/Services/TriggerPointGenerator.cs` — namespace `ChefKnifeStudios.TransitJazz.Shared.Services`, keep `TriggerSpacingMeters = 400.0` and all logic unchanged, update `using`s; delete `src/Client/ChefKnifeStudios.TransitJazz.Client.Shared/Services/TriggerPointGenerator.cs` (depends on T002, T003).
- [X] T005 [P] Create `src/ChefKnifeStudios.TransitJazz.Shared/Events/RouteCrossingBatchEvent.cs` — `sealed record RouteCrossingBatchEvent(IEnumerable<RouteCrossingRecord> BatchRecords) : ISignalREvent` with nested `RouteCrossingRecord(string VehicleId, string RouteId, int TriggerIndex, int TotalTriggers)`, per `contracts/route-crossing-event.md`.
- [X] T006 Fix client references to the moved types: update `using` directives in `src/Client/ChefKnifeStudios.TransitJazz.Client.WebApp/Pages/TransitMap.razor.cs` and `src/Client/ChefKnifeStudios.TransitJazz.Client.WebApp/Program.cs` (and any other consumer surfaced by build) to `ChefKnifeStudios.TransitJazz.Shared.Models` / `...Shared.Services`; keep the `ITriggerPointGenerator` DI registration (now the Shared type). Build the solution to surface all break sites (depends on T002–T004).
...
- [X] T007 [P] [US1] Server/client trigger-point equality test in `src/Server/ChefKnifeStudios.TransitJazz.Server.TransitDataWorker.Tests/TriggerPointEqualityTests.cs` — for a representative route geometry, assert `TriggerPointGenerator.Generate(coords, cumDist)` count and `(Index, AlongDistanceM)` sequence are stable (pins that the move changed nothing). (quickstart Test 7 / SC-006)
- [X] T008 [P] [US1] `CrossingDetector` unit tests in `src/Server/ChefKnifeStudios.TransitJazz.Server.TransitDataWorker.Tests/CrossingDetectorTests.cs` — synthetic `cumDist` + `triggerPoints` + scripted snap-index sequence; assert emitted `triggerIndex` sets for: first observation (none), forward across 1, forward across many, backward (none), teleport >2000m (none), route transfer (none), forward-with-no-new-trigger (none). (FR-007..FR-011 / SC-003)
...
- [X] T016 [US2] Delete the client local-detection files: `src/Client/ChefKnifeStudios.TransitJazz.Client.Shared/wwwroot/js/checkpoint-tracker.js`, `.../Services/JsInterop/CheckpointTrackerJsInterop.cs`, and `.../Services/JsInterop/ICheckpointTrackerJsInterop.cs` (depends on T015).
- [X] T017 [US2] In `src/Client/ChefKnifeStudios.TransitJazz.Client.WebApp/Program.cs`, remove the `ICheckpointTrackerJsInterop` → `CheckpointTrackerJsInterop` DI registration; confirm the `ITriggerPointGenerator` registration remains (Shared type, still used for markers) (depends on T016).
...
Task: "T005 Create RouteCrossingBatchEvent in src/ChefKnifeStudios.TransitJazz.Shared/Events/RouteCrossingBatchEvent.cs"
